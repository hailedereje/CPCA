...
export const CodeContentEditor = ({ topicItem }) => {
    ...
    return (
        <div className={`fixed z-20 flex items-center justify-center top-0 left-0 w-screen h-screen  transform transition-all duration-500 ${show ? 'scale-100 ' : 'scale-40 hidden'}`}>
            <div onClick={() => dispatch(toggleShow(({ id: topicItem.id })))} className="absolute top-0 left-0 w-full h-full bg-black/60" />
           
            <div className="flex flex-col items-start justify-between z-10 gap-4 w-fit duration-300 bg-white p-3 rounded-md">
                <div className="dropdown">
                    <div tabIndex={0} role="button" className="btn m-1 px-4 ">{code.language || 'javascript'}</div>
                ...
                <div className="flex gap-5">
                    <button onClick={() => {
                        dispatch(updateTopic({ id, content: code }))
                    ...
                    <button onClick={() => dispatch(toggleShow(({ id })))}>cancel</button>
                </div>
