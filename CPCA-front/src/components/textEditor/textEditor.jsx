...
import { addTopic, removeTopic, toggleShow,addImage } from "../../features/course/topicSlice";
...
function RichTextExample() {
  ...
  const topic = useSelector(state => state.topicState.topics)
  ...
  const handleUpload = (e, idx) => {
    ...
    axios.post(
      "https://api.cloudinary.com/v1_1/dygzy1vri/image/upload",
      formData
    )
      .then((response) => {
        console.log("uploaded successfully", response);
        // setCloudImage(response.data.secure_url);
        addImage(idx, response.data.secure_url)
        setImage(null)
      })
      .catch((error) => {
        console.log(error);
  ...
  return (
    <div className="flex flex-col items-center  gap-10 my-10">
      ...
      {topic.map((topicItem, idx) => (
        ...
        <div className="group w-full h-full flex items-end justify-center gap-5" key={topicItem.id}>
          <EditLinks idx={idx} />
          {topicItem.name === "text" &&
            <>
              <div className="w-4/5 group  flex flex-col-reverse gap-y-4  bg-slate-50/45 group-hover:bg-slate-50 rounded-lg ">
                <div className="flex items-center justify-end gap-5 p-3 invisible group-hover:visible">
                  <button onClick={() => dispatch(toggleShow({id:topicItem.id}))} className="">
                ...
                <div className="p-3 max-h-[300px] min-h-[100px] overflow-auto">
                  {parser(topicItem.content)}
                </div>
              </div>
              <TextEditor topicItem={topicItem} />
            </>
          }
          {
            ...
            <>
              <div className="w-4/5 group  flex flex-col-reverse gap-y-4 bg-slate-50/45 group-hover:bg-slate-50 rounded-lg ">
                <div className="flex items-center justify-end gap-5 p-3 invisible group-hover:visible">
                  <button onClick={() => toggleEditor(topicItem.id)} className="">
                    <svg className="w-6 text-blue-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                      <path fill="currentColor" d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10s10-4.47 10-10S17.53 2 12 2m3.1 5.07c.14 0 .28.05.4.16l1.27 1.27c.23.22.23.57 0 .78l-1 1l-2.05-2.05l1-1c.1-.11.24-.16.38-.16m-1.97 1.74l2.06 2.06l-6.06 6.06H7.07v-2.06z" />
                    </svg>
                  </button>
                ...
                <div className="p-3 max-h-[300px] min-h-[100px] overflow-auto">
                  <img src={topicItem.content} className="object-contain max-w-[70%]" alt="" />
                </div>
              </div>

            </>
          }
          <UploadImage idx={idx} topic={topic}/>


          {topicItem.name === 'code' &&
            <>
             <CodeContentEditor topicItem={topicItem}/>
              <div className="w-4/5 group  flex flex-col-reverse gap-y-4 bg-slate-50/45 group-hover:bg-slate-50 rounded-lg ">
                <div className="flex items-center justify-end gap-5 p-3 invisible group-hover:visible">
                  <button onClick={() => dispatch(toggleShow(({id: topicItem.id})))} className="">
                    <svg className="w-6 text-blue-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                ...
                <div className="flex items-center p-3 w-full h-full max-h-[300px] min-h-[100px] overflow-auto">
...
const EditLinks = ({idx }) => {
  ...
  const initialTopic = { name: "text",order:0, id: nanoid(), content: "", show: false }
  return (
    <div className="invisible group-hover:visible w-fit flex flex-col items-end gap-5 relative">
      <div className={`absolute -top-10 flex justify-between gap-4 transition-opacity duration-300 ${buttonsVisible ? 'opacity-100' : 'opacity-0'} `}>
        <button onClick={() =>dispatch(addTopic({idx,topic:{...initialTopic,name:'code',content:{}}}))}>
        ...
        <button onClick={() =>dispatch(addTopic({idx,topic:initialTopic}))}>
          <svg className="w-6 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <input type="file" name="" id="image-file" hidden onChange={(e) => dispatch(addImage({image:e.target.files[0],idx}))} />
      </div>

      <button className="" onClick={toggleButtonsVisibility}>
        <svg className={`w-8 text-green-600 transform transition-transform duration-300 ${buttonsVisible ? 'rotate-45' : 'rotate-0'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
