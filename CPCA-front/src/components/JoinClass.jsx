  ...
  const handleJoinClass = async () => {
    try {
      ...
      if (error.response.data.msg === "User not found") {
        ...
        toast.error("You have already joined this classroom");
        ...
        toast.error("Failed to join classroom, Something went wrong");
      }
    }
  };
  ...
  return (
    <div className="flex flex-col items-center h-screen justify-center">
      <div className="p-4 flex flex-col text-center w-96 border border-gray-300 rounded-t-lg pb-10 shadow-lg hover:shadow-xl">
        ...
        <p className="">
          Hey <span className="font-bold">{data.username}</span>, you are
          invited to join{" "}
          <span className="font-bold text-lg">{data.classroomName}</span> class
        </p>
        <p>
          by Instructor{" "}
          <span className="font-bold text-lg">{data.instructor}</span>
        </p>
      </div>
      <button
